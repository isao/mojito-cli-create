...
function handleBareFileDir(from, to) {
    ...
    // so if the --directory option is used, we may need to mkdirp it.
    if (is_file && !utils.exists(destdir)) {
        log.debug('mkdirp %s for file %s', destdir, path.basename(to));
        mkdirp(destdir);
    }
}

// todo: enable config TEMPLATE_RE, ignores
function main(from, to, data, cb) {
    var scan = new Scan(ignore), errs = [];

    // special case where "from" is a file, and --directory option is used
