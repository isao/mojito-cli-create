...
function replacer(kvs, rgx, eol) {
    ...
    function replaceCb(match, key) { // mdn http://cl.ly/OPzu
        if(kvs.hasOwnProperty(key) && key == 'name') {
            return String(kvs[key]).toLowerCase();
        } else {
           return kvs[key] || match;
        }
    }
    ...
    function end() {
        /*jshint eqnull:true*/
        if (sofar != null) {
            // restore eol that was removed with .split(eol) & queue last bit
            this.queue(eol + sofar);
        }
        this.queue(null);
    }
